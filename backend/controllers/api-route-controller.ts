...
import { BodyRequest, QueryRequest, ParamsRequest } from "@/types/requests";
...
export abstract class ApiRouteController {
  ...
  protected setUpMiddleware(handler: ApiRouteHandler): ApiRouteHandler {
    ...
    if (this.params) {
      this.middlewares.unshift(validateParamMiddleware(this.params.constructor));
    }
  ...
  protected setupHandlers(handler: ApiRouteHandler): ApiRouteHandler {
    return this.requestTypes.reduce((handler: ApiRouteHandler, type: RequestType) => {
      const func = async (req: NextApiRequest, res: NextApiResponse<any>) => {
        try {
          this.body = req.data.body;
          this.query = req.data.query;
